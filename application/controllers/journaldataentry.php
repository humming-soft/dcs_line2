...
class Journaldataentry extends CI_Controller
{
    ...
    function index($offset=0)
    {
        ...
        if($this->session->userdata('logged_in'))
        {
            ...
            $data['pjdefreq'] = array ();
            $data['status_span'] = array ();
            $cweek=Date('W')+1;
            // echo 'week';
            // echo ($cweek);
            foreach ( $data['records'] as $k => $freq )
            {
                if($freq->journal_category_id==1){
                    $status=0;
                    $leftpier=$this->design->get_span_pier_id($freq->journal_no);
                    $rightpier=$this->design->get_span_pier_id_two($freq->journal_no);
                    if($leftpier >0 && $rightpier > 0){
                        $leftpier_uid=$this->design->get_pier_name($leftpier);
                        $rightpier_uid=$this->design->get_pier_name($rightpier);
                        $chk_status_lid=$this->design->get_left_pier_uid_status($leftpier_uid);
                        $chk_status_rid=$this->design->get_left_pier_uid_status($rightpier_uid);
                        if($chk_status_lid==1 && $chk_status_rid==1 ){
                            $status=1;
                        }else{
                            $status=0;
                        }
                        ...
                        $leftpier_uid=$this->design->get_span_pier_id($freq->journal_no);
                        $leftpier_uid=$this->design->get_pier_name($leftpier_uid);
                        $chk_status_lid=$leftpier_uid=$this->design->get_left_pier_uid_status($leftpier_uid);
                        if($chk_status_lid==1){
                            $status=1;
                        }else{
                            $status=0;
                    ...
                    $data['status_span'][$freq->journal_no]=$status;
                }else if($freq->journal_category_id==4){
                    $span=$this->design->get_span_journal_id($freq->journal_no);
                    $chk_status_spanid=$this->design->get_span_journal_status($span);
                    if($chk_status_spanid==1){
                        $status=1;
                    }else{
                        $status=0;
                    }
                    ...
                    $data['status_span'][$freq->journal_no]=2;
                }
                $pjde="";
                $isthisweek = false;
