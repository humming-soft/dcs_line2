...
//        $('#dag').DataTable();
		...
		$(document).on("click", ".modaladd", function ()
		{
			...
			$("#errordataattrgrpdesc").html( empty );
		});
		...
		$('#addrecord').submit(function()
		{
			$.post($('#addrecord').attr('action'), $('#addrecord').serialize(), function( data )
			{
				...
				{
					hideloader();
					$('#errordataattrgrpdesc').html(data.msg);
				}
				else if(data.st == 1)
				{
		...
		{
			var dataattrgrpdesc = $(this).data('dataattrgrpdesc');
			var dataattrgrpid = $(this).data('dataattrgrpid');
			var empty ="";
			$(".modal-body #dataattrgrpdesc1").val( dataattrgrpdesc );
			$(".modal-body #dataattrgrpid").val( dataattrgrpid );
			$("#errordataattrgrpdesc").html( empty );
		});
		...
		$('#updaterecord').submit(function()
		{
			$.post($('#updaterecord').attr('action'), $('#updaterecord').serialize(), function( data )
			{
				...
				{
					hideloader();
		 			$('#errordataattrgrpdesc1').html(data.msg);
		...
		$(document).on("click", ".modaldelete", function ()
		{
			...
			{
				var dataattrgrpid = $(this).data('id');
				$.post( "<?php echo base_url(); ?><?php echo $cpagename; ?>/delete",{dataattrgrpid:dataattrgrpid}, function( data ) {
					location.reload();
				});
			}
		});
				...
				<form method=post id=addrecord action="<?php echo base_url(); ?><?php echo $cpagename; ?>/add/">
					<div class="modal-body">
						<div class="form-group">
							...
							<div class="col-md-8">
								<label id="errordataattrgrpdesc" class="text-danger"></label>
							</div>
						</div>
						<br>
						<div class="row">
							<div class="form-group">
								...
								<div class="col-sm-4">
									<input type="text" class="form-control" name="dataattrgrpdesc"  id="dataattrgrpdesc" placeholder="GROUP NAME" maxlength="40">
								</div>
							</div>
						</div>
						<br>
					...
					<div class="modal-body">
						<div class="row">
							<div class="form-group">
								<label for="select" class="col-md-3 control-label"></label>
								<div class="col-md-8">
									<label id="errordataattrgrpdesc1" name="errordataattrgrpdesc1" class="text-danger" ></label>
								</div>
							</div>
						</div>
						<br>
						<div class="row">
							<div class="form-group">
								<label for="dataattrgrpdesc1" class="col-sm-3 control-label"><?php echo $labelname[0]; ?><red>*</red></label>
								<div class="col-sm-4">
									<input type="text" class="form-control" name="dataattrgrpdesc1"  id="dataattrgrpdesc1" value="" maxlength="40">
									<input type="hidden" class="form-control" name="dataattrgrpid"  id="dataattrgrpid">
								</div>
							</div>
						</div>
						<br>
	...
	<div class="row">
		<table class="table table-striped table-hover" id="dag">
			...
			<tbody>
				<?php
					$sno=1;
					foreach ($records as $dataattrgrp):
				?>
						<tr>
							<td><?php echo $sno; ?></td>
							<td><?php echo html_escape($dataattrgrp->data_attribute_group_desc); ?></td>
							<td>
								<?php
									if($editperm==1)
									{
								?>
										<a href="#" data-toggle="modal" class="modaledit" data-target="#myModalEdit" data-dataattrgrpdesc="<?php echo html_escape($dataattrgrp->data_attribute_group_desc); ?>" data-dataattrgrpid="<?php echo html_escape($dataattrgrp->data_attribute_group_id); ?>"><span class="glyphicon glyphicon-edit">&nbsp;</span></a>
								<?php
									}
									else
									{
		          			...
		          			<td>
		          				<?php
									...
									{
								?>
										<a href="#" data-toggle="modal" class="modaldelete" data-id="<?php echo html_escape($dataattrgrp->data_attribute_group_id); ?>"><span class="glyphicon glyphicon-trash">&nbsp;</span></a>
								<?php
									}
									else
									{
