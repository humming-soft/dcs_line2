	...
	if($data=="journalentry")
	{
		...
		if(pg_num_rows($q)==0)
		{
			...
			while($myrow = pg_fetch_assoc($res))
			{
				...
				$query .=" values('$id','".$myrow['data_attb_id']."','".$myrow['start_value']."','".$myrow['start_value']."','".$myrow['end_value']."','".$myrow['frequency_max_value']."','".$myrow['display_seq_no']."',2,'$loginid','".date("Y-m-d")."')";
				pg_query($query);
			}
